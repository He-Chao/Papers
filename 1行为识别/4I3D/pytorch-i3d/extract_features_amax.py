...
parser.add_argument('-root',default='/home/amax/renpengzhen/data')
# parser.add_argument('-root',default='/home/r/renpengzhen/Datasets/Charades_v1_rgb/Charades_v1_rgb', type=str) #实验室服务器
parser.add_argument('-data_root',default='Charades_v1_rgb', type=str) #外购服务器
...
args.save_dir = os.path.join(args.root,args.save_dir)





def run(max_steps=64e3, mode='', root='', split='charades/charades.json', batch_size=1, load_model='', save_dir='',ts = 1600):
    # ts = 800  # 原版为1600,划分边界
    
    # setup dataset
    test_transforms = transforms.Compose([videotransforms.CenterCrop(224)]) #对视频数据进行中心裁剪，大小为224
    
    save_dir = save_dir + '%d' % ts
        ...
        for data in dataloaders[phase]:
            ...
            if t > ts:
                ...
                for start in range(1, t-56, ts):
                    ...
                    # ip = Variable(torch.from_numpy(inputs.numpy()[:, :, start:end]).cuda(), volatile=True) #原始的代码，但是报错
                    with torch.no_grad():
                        ip = Variable(torch.from_numpy(inputs.cpu().numpy()[:,:,start:end]).cuda())
                        features.append(i3d.extract_features(ip).squeeze(0).permute(1,2,3,0).data.cpu().numpy())#提取特征
            ...
            i += 1
    count_number_of_pre_video_frames(save_dir, ts)

def count_number_of_pre_video_frames(path,ts):
    ...
    for root, dir, files in os.walk(path):
        ...
        for file in files:
            ...
            idx += 1
    # ts = 800
    save_path = './extract_feature_count_%d.npy'%ts
...
if __name__ == '__main__':
    run(mode=args.mode, root=args.data_root, save_dir=args.save_dir, load_model=args.load_model, ts=400)
