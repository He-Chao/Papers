...
def run_demo(args):
    kinetics_classes = [x.strip() for x in open(args.classes_path)] #读取label

    def get_scores(sample, model):
        sample_var = torch.autograd.Variable(torch.from_numpy(sample).cuda()) #导入样本，并进行封装
        out_var, out_logit = model(sample_var) #输入到model中进行预测
        out_tensor = out_var.data.cpu()

        top_val, top_idx = torch.sort(out_tensor, 1, descending=True) #对输出的结果进行排序

        print('Top {} classes and associated probabilities: '.format(args.top_k))
        for i in range(args.top_k):
            print('[{}]: {:.6E}'.format(kinetics_classes[top_idx[0, i]],top_val[0, i]))
        return out_logit

    # Rung RGB model
    if args.rgb:
        i3d_rgb = I3D(num_classes=400, modality='rgb') #导入模型
        i3d_rgb.eval() #开启模型的预测模式
        i3d_rgb.load_state_dict(torch.load(args.rgb_weights_path)) #加载模型参数
        i3d_rgb.cuda()

        rgb_sample = np.load(args.rgb_sample_path).transpose(0, 4, 1, 2, 3) #(1, 3，79, 224, 224) #3是通道数，79是帧数
        out_rgb_logit = get_scores(rgb_sample, i3d_rgb) #计算得分
    ...
    if args.flow:
        ...
        flow_sample = np.load(args.flow_sample_path).transpose(0, 4, 1, 2, 3) #(1, 2, 79, 224, 224)
        out_flow_logit = get_scores(flow_sample, i3d_flow)

    # Joint model联合两种模式进行预测
...
if __name__ == "__main__":
    ...
    parser.add_argument('--flow_weights_path',type=str,
                        default='model/model_flow.pth',
                        help='Path to flow model state_dict')
    parser.add_argument('--flow_sample_path',type=str,
...
#运行
#python i3d_pt_demo.py --rgb --flow