...
def _get_image_blob(im):
    ...
    for target_size in cfg.TEST.SCALES:
        ...
        if im.shape[0]<600 or im.shape[1]<600:
            if im.shape[0]<600:
                pad_size=np.round((600-im.shape[0])/2)
                np.pad(im,((pad_size,pad_size),(0,0),(0,0)))
            else:
                pad_size=np.round((600-im.shape[1])/2)
                im=np.pad(im, ((0,0), (pad_size, pad_size), (0,0)),'constant')
...
def _get_blobs(im, rois):
    ...
    #50:数据类型都是np.ndarray,shape=((1, 3, 1000, 600), (1,))
    #101：也是，没有问题
    # print(cfg.TEST.HAS_RPN)#是ture
...
def im_detect(net, im, boxes=None, force_boxes=False):
    ...
    im_blob = blobs['data'] #(1, 3, 1000, 600)
    blobs['im_info'] = np.array(
        [[im_blob.shape[2], im_blob.shape[3], im_scales[0]]],
        dtype=np.float32) #[[1000,600,2.849...]]
