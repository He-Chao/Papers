...
class BaseOptions(object):
    ...
    def __init__(self):
        ...
        self.parser.add_argument('--save_path', type=str, default='../runs/debug', help='Folder to save checkpoints and log.')
        self.parser.add_argument('--pretrain', type=str, default=False,help='Whether to pre-train')
    ...
    def parse(self):
        ...
        with open(os.path.join(self.args.save_path, 'args.yaml')) as f:
            extra_args = yaml.load(f, Loader=yaml.FullLoader)
...
class Logger(object):
    ...
    def print_log(self, string):
        self.file.write("{}\n".format(string)) #向文件中写入字符串
        self.file.flush() #用来刷新缓冲区的，即将缓冲区中的数据立刻写入文件，同时清空缓冲区，不需要是被动的等待输出缓冲区写入。
        print(string)
...
class DiscriminativeLoss(torch.nn.Module):
    ...
    def init_threshold(self, pairwise_agreements):
        pos = int(len(pairwise_agreements) * self.mining_ratio) #取的困难负例对的个数，0.5%
        sorted_agreements = np.sort(pairwise_agreements) #从小到大进行排序，pairwise_agreements值越大说明相似度越高
        t = sorted_agreements[-pos] #阈值
        self.register_buffer('threshold', torch.Tensor([t]).cuda())
        self.initialized = True

    def forward(self, features, multilabels, labels):
        """
        :param features: shape=(BS, dim)=(100,2048)
        :param multilabels: (BS, n_class)
        :param labels: (BS,)
        :return:
        """
        P, N = self._partition_sets(features.detach(), multilabels, labels) #划分正例对P与困难负例对N
        if P is None:
    ...
    def _partition_sets(self, features, multilabels, labels):
        ...
        p_dist = pdist(f_np) #计算特征之间的距离，表示相似度
        p_agree = 1 - pdist(ml_np, 'minkowski', p=1) / 2 #软多标签之间的相似度
        ...
        pos_idx = similar_idx[is_positive] #正例对的下标
        neg_idx = similar_idx[~is_positive] #困难负例对的下标
    ...
    def _update_buffers(self, P, labels):
        ...
        rate_TP = float(count) / n_pos_pairs #预测是正例对的正确率
...
class JointLoss(torch.nn.Module):
    ...
    def forward(self, features, agents, labels, similarity, features_target, similarity_target):
        """
        :param features: shape=(BS/2, dim),(100,2048)，辅助数据集的特征
        :param agents: shape=(n_class, dim),(4101,2048)
        :param labels: shape=(BS/2,)
        :param features_target: shape=(BS/2, n_class)
        :return:
        """
        loss_terms = []
        arange = torch.arange(len(agents)).cuda() #代理的个数，等于辅助数据集的类别数
        zero = torch.Tensor([0]).cuda()
        for (f, l, s) in zip(features, labels, similarity):
            loss_pos = (f - agents[l]).pow(2).sum() #和预测正确的那个代理求平方差损失
            loss_terms.append(loss_pos)
            neg_idx = arange != l #防止下标越界
            hard_agent_idx = neg_idx & (s > self.sim_margin) #得到相似度大于1/2的代理的下标
...
class MultilabelLoss(torch.nn.Module):
    #跨视角一致性损失，CML_loss,同一个摄像头得到的多标签应该具有一致性
    ...
    def _update_centers(self, log_multilabels, views):
        ...
        :return:
        """
        univiews = torch.unique(views) #摄像头的个数
        means = []
        stds = []
        for v in univiews:
            ml_in_v = log_multilabels[views == v]
            if len(ml_in_v) == 1:
                continue
            mean = ml_in_v.mean(dim=0) #按列求均值,[1,4101]
            means.append(mean)
            if self.use_std:
                std = ml_in_v.std(dim=0)
                stds.append(std)
        new_mean = torch.mean(torch.stack(means), dim=0)
        self.center_mean = self.center_mean * (1 - self.moment) + new_mean * self.moment #更新均值,self.moment = 0.01
        if self.use_std:
            new_std = torch.mean(torch.stack(stds), dim=0)
            self.center_std = self.center_std * (1 - self.moment) + new_std * self.moment #更新方差
    ...
    def forward(self, log_multilabels, views):
        ...
        loss_terms = []
        for v in univiews:
            ml_in_v = log_multilabels[views == v] #是第v个摄像头中的软标签
            if len(ml_in_v) == 1:
                #如果第v个摄像头下的软标签个数只有1个，取下一个v+1
                continue
            mean = ml_in_v.mean(dim=0)
...
def extract_features(loader, model, index_feature=None, return_numpy=True):
    ...
    for i, data in enumerate(loader):
        ...
        for m, feat_idx in zip(models, indices_feature):
            ...
            feature_batch = output_tuple if feat_idx is None else output_tuple[feat_idx] #这里是输出sim
            inputs = feature_batch
        # print(type(feature_batch), feature_batch.shape) #<class 'torch.Tensor'> torch.Size([100, 4101])
...
def partition_params(module, strategy, *desired_modules):
    ...
    if strategy == 'bn':
        ...
        for m in module.modules():
            if (isinstance(m, torch.nn.BatchNorm1d) or
                ...
                desired_params_set.update(set(m.parameters())) #向集合中更新元素
    ...
    all_params_set = set(module.parameters()) #构造成无序不重复的参数集合
    other_params_set = all_params_set.difference(desired_params_set) #差集，包含在第一个集合但不包含在第二个集合
...
def get_transfer_dataloaders(source, target, img_size, crop_size, padding, batch_size, use_source_stat=True):
    ...
    # MSMT17.mat：124068张图片，4101个人
    source_data = FullTraining('../data/{}.mat'.format(source)) #辅助数据集MSMT17.mat,制作数据集,
    # Market.mat: 32217,1501个人
    target_data = Market('../data/{}.mat'.format(target), state='train') #(12936, 3, 128, 384)
    gallery_data = Market('../data/{}.mat'.format(target), state='gallery') #(15913, 3, 128, 384)
    probe_data = Market('../data/{}.mat'.format(target), state='probe') #(3368, 3, 128, 384)
    ...
    transforms.Resize(img_size):重整像素到0-1之间
    transforms.RandomCrop(crop_size, padding):先填充在进行随机裁剪
    transforms.Normalize(mean, std):对数据按通道进行标准化
    '''
    if not use_source_stat:
...
def parse_pretrained_checkpoint(checkpoint, num_classes, fc_layer_name='fc'):
    ...
    fc_weight = state_dict[fc_weight_name] #fc的权重
