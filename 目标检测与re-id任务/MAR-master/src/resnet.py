...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, pretrain=False):
        self.inplanes = 64
        super(ResNet, self).__init__()
        self.pretrain = pretrain #自己重新进行预训练
    ...
    def forward(self, x):
        ...
        feature_maps = self.layer4(x) #torch.Size([100, 2048, 12, 4])
        x = self.avgpool(feature_maps) #torch.Size([100, 2048, 1, 1])
        x = x.view(x.size(0), -1)
        feature = x.renorm(2, 0, 1e-5).mul(1e5)  # 将每行进行l2范数归一化，torch.Size([100, 2048])
        if self.pretrain:
            w = self.fc.weight
            sim = x.mm(w.t())
        else:
            w = self.fc.weight #torch.Size([4101, 2048])
            ww = w.renorm(2, 0, 1e-5).mul(1e5) #torch.Size([4101, 2048])，这个其实就是agents
            sim = feature.mm(ww.t()) #这里采用的是余弦相似度，由于是l2进行了正则化，因此分母=1，矩阵相乘,[100,4101]
